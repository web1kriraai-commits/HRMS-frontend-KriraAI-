...
import { LayoutDashboard, Users, Settings, LogOut, CheckSquare, Calendar, CalendarDays, UserCircle, TrendingUp, Clock, FileText, Activity, Globe, BookOpen, IndianRupee, ClipboardList } from 'lucide-react';
  ...
  return (
    <div className="w-64 bg-slate-900 text-white h-screen fixed left-0 top-0 flex flex-col shadow-xl">
      ...
      <div className="flex-1 py-6 px-3 space-y-1">
        ...
        <NavLink to="/" end className={({ isActive }) => `flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${isActive ? 'bg-blue-600 text-white' : 'text-slate-300 hover:bg-slate-800'}`}>
          <LayoutDashboard size={18} />
          Dashboard
        </NavLink>

        {user.role === Role.HR && (
          <NavLink to="/hr-dashboard" className={({ isActive }) => `flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${isActive ? 'bg-blue-600 text-white' : 'text-slate-300 hover:bg-slate-800'}`}>
            <CheckSquare size={18} />
            HR Dashboard
          </NavLink>
        )}
        ...
        {isAdminOrHr && (
          <>
            ...
            <NavLink to="/admin-employee-summary" className={({ isActive }) => `flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${isActive ? 'bg-blue-600 text-white' : 'text-slate-300 hover:bg-slate-800'}`}>
